...
| Picker tab bar | `bubbles/v2` | v2.1.1 | keep-custom | Bubbles tabs do not render a three-label ASCII bar with reverse active and muted inactive states under picker chrome width rules. | `TestFormatTabBarActiveReverseInactiveMuted` |
...
The picker needs a three-label ASCII bar with reverse on the active tab and muted inactive tabs, clipped to chrome width. Bubbles tabs do not match that chrome. Keep `FormatTabBar`.
