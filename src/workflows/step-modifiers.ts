...
export function rejectV1Placeholders(
  ...
  if (input) {
    throw new WorkflowLoadError(
      positioned(file, stepIndex, key, `'{input.${input}}' is removed — use {${input}}`),
    );
  }
}
...
function rejectShellPlaceholders(
  ...
  if (firstPlaceholder(text)) {
    throw new WorkflowLoadError(positioned(file, stepIndex, key, PLACEHOLDER_SHELL));
  }
}
...
export function parseGuard(file: string, stepIndex: number, key: string, value: unknown): Guard {
  if (Array.isArray(value)) {
    if (!value.every((v) => typeof v === "string")) {
      throw new WorkflowLoadError(
        positioned(file, stepIndex, key, "argv guard elements must be strings"),
      );
    }
  ...
  if (typeof value !== "string") {
    throw new WorkflowLoadError(
      positioned(file, stepIndex, key, "when:/until: must be a string or argv list"),
    );
  }
...
export function parseFor(file: string, stepIndex: number, value: unknown): ForSource {
  if (Array.isArray(value)) {
    if (value.length === 0) {
      throw new WorkflowLoadError(positioned(file, stepIndex, "for", "for: list is empty"));
    }
    if (!value.every((v) => typeof v === "string")) {
      throw new WorkflowLoadError(
        positioned(file, stepIndex, "for", "for: list elements must be strings"),
      );
    }
  ...
  if (typeof value !== "string") {
    throw new WorkflowLoadError(
      positioned(file, stepIndex, "for", "for: must be a list, sh <cmd>, or {name}"),
    );
  }
...
export function parseRetry(file: string, stepIndex: number, value: unknown): RetrySpec {
  if (typeof value === "number") {
    if (!Number.isInteger(value) || value < 1) {
      throw new WorkflowLoadError(positioned(file, stepIndex, "retry", "times must be at least 1"));
    }
    return { times: value };
  }
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new WorkflowLoadError(
      positioned(file, stepIndex, "retry", "retry: must be an integer or map"),
    );
  }
  ...
  if (map.delay !== undefined) {
    if (typeof map.delay !== "number" || map.delay < 0) {
      throw new WorkflowLoadError(
        positioned(file, stepIndex, "retry", "delay must be a non-negative number"),
      );
    }
  ...
  if (map.reset !== undefined) {
    if (typeof map.reset !== "string") {
      throw new WorkflowLoadError(
        positioned(file, stepIndex, "retry", "reset: must be a shell command string"),
      );
    }
...
export function parseWait(file: string, stepIndex: number, value: unknown): WaitSpec {
  ...
  throw new WorkflowLoadError(
    positioned(file, stepIndex, "wait", "wait: must be true, false, or /regex/"),
  );
}
...
export function parseOut(file: string, stepIndex: number, value: unknown): OutSpec {
  if (typeof value === "string") {
    if (!/^[a-z][a-z0-9_]{0,31}$/.test(value)) {
      throw new WorkflowLoadError(
        positioned(file, stepIndex, "out", "out: name must match [a-z][a-z0-9_]{0,31}"),
      );
    }
  ...
  if (value && typeof value === "object" && !Array.isArray(value)) {
    ...
    for (const [name, path] of Object.entries(value as Record<string, unknown>)) {
      if (!/^[a-z][a-z0-9_]{0,31}$/.test(name)) {
        throw new WorkflowLoadError(
          positioned(file, stepIndex, "out", `out: name '${name}' must match [a-z][a-z0-9_]{0,31}`),
        );
      }
      if (typeof path !== "string" || !path) {
        throw new WorkflowLoadError(
          positioned(file, stepIndex, "out", `out.${name} must be a dot-path string`),
        );
      }
  ...
  throw new WorkflowLoadError(
    positioned(file, stepIndex, "out", "out: must be an identifier or map"),
  );
}
...
export function parseRunPayload(file: string, stepIndex: number, step: RawStep): RunPayload {
  ...
  if (!(SHELLS as readonly string[]).includes(shell)) {
    throw new WorkflowLoadError(
      positioned(file, stepIndex, "shell", `shell: must be one of ${SHELLS.join(", ")}`),
    );
  }
  if (Array.isArray(value)) {
    if (!value.every((v) => typeof v === "string")) {
      throw new WorkflowLoadError(
        positioned(file, stepIndex, "run", "argv elements must be strings"),
      );
    }
  ...
  if (typeof value !== "string") {
    throw new WorkflowLoadError(
      positioned(file, stepIndex, "run", "run: must be a string or argv list"),
    );
  }
