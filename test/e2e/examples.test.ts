...
/** Each test spawns the real CLI several times, so Bun's 5s default is too tight. */
...
describe("shipped examples through isolated CLI and fake Herdr", () => {
  test(
    "branch-check covers clean, changed, existing, and available outcomes",
    async () => {
      const scenarios: {
  ...
  test(
    "prompt-enhance uses the configured custom agent and this platform's clipboard",
    async () => {
  ...
  test(
    "handoff preserves the target and cleans up at the selected granularity",
    async () => {
      for (const placement of ["tab", "beside", "below"]) {
        for (const closeSource of ["keep", "close"]) {
          ...
          expect(
            calls.some(
              (call) => call.method === "pane.close" && call.params.pane_id === targetPane,
            ),
          ).toBe(false);
          if (closeSource === "keep") {
  ...
  test(
    "worktree create runs the full layout with a per-pane agent name",
    async () => {
      const calls = successful(
  ...
  test(
    "worktree open targets an existing branch through the same layout",
    async () => {
      const calls = successful(
  ...
  test(
    "review-gate notifies on APPROVE and fails the run on REJECT",
    async () => {
  ...
  test(
    "review-gate skips the reviewer when the diff is empty",
    async () => {
      const calls = successful(
  ...
  test(
    "adversarial-revise runs the revision step only on the REVISE verdict",
    async () => {
      const scenarios = [
  ...
  test(
    "remote-branch-log resolves the branch choice from the chosen remote",
    async () => {
  ...
  test(
    "worktree open skips agent.start when the reopened pane already has one",
    async () => {
      const calls = successful(
        await harness.run(
          "worktree",
  ...
  test(
    "worktree delete removes the checkout and reports the branch outcome",
    async () => {
