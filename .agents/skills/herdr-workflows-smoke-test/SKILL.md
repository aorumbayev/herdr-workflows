...
**Shared binary limit:** `up` still runs `go build -o bin/herdr-workflows ./cmd/herdr-workflows` and then `bin/herdr-workflows setup` in this checkout. That rebuilds shared `bin/herdr-workflows`. The user's live Herdr picks up the same binary on its next plugin action. Config, socket, and panes are isolated. The plugin binary is not.
...
```bash
repo_root=$(git rev-parse --show-toplevel)
bash "$repo_root/.agents/skills/herdr-workflows-smoke-test/scripts/picker-visual-compare.sh" all
```

| Side | Sandbox root | Herdr tmux | Picker UI tmux | Binary |
| ---- | ------------ | ---------- | -------------- | ------ |
