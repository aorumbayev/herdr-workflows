...
This skill produces one YAML file. `hwf` (≡ `herdr-workflows`, a herdr plugin) runs that file as a
linear sequence of steps. Format is `version: v1alpha1`. This skill and its reference files contain
everything you need. A key that is not in these files does not exist in v1alpha1 (rule 13 lists the
usual inventions).
...
`<name>`: lowercase, `[a-z0-9][a-z0-9-_]*` (hyphens allowed). Step `id:` uses a _different_,
stricter rule. Refer to the Rules section. Edit in `$EDITOR` (picker `n`/`o`) or any text editor.
Run with `prefix+k` or `hwf run <name> [--input k=v]`.
...
Workflow YAML is reviewed executable code. There is no sandbox. Prefer one step that works over a
...
`workflow:` + `inputs:` rather than duplicate steps.

**[reference/herdr-api.md](reference/herdr-api.md)** lists the `herdr:` methods and their required
selectors. That list is complete, so use it. It also names the runtime fallback for a method that
the table does not include. This skill lives outside the herdr-workflows checkout, so its `src/`
and `schemas/` are **not readable**.
...
Run it **from the project root** (the directory that holds `.hwf/`), with an absolute path to the
script. The loader resolves `workflow:` children and `using:` profiles relative to the current
directory. The loader validates dynamic `options: {run:}` declarations but does not run discovery.
Validate from the project root so child and profile resolution uses the project configuration.
Validation from this skill's install directory can report false errors such as
`workflow '<child>' not found`.
...
when you call the CLI directly.
...
Do **not** hand-write a `# yaml-language-server: $schema=…` line that points at `main`. Pin the
...
1. **Step `id:` must match `[a-z][a-z0-9_]{0,31}`** — lowercase, starts with a letter, `_` only.
   The loader rejects hyphens: `id: check-lint` fails to load, use `check_lint`. **Input names use
   the same rule.** Workflow _file_ names use the looser `[a-z0-9][a-z0-9-_]*` — do not confuse the two.
2. **No templates in string `run:` text.** Use argv `run: [git, checkout, "{{inputs.branch}}"]` or `env:` / `$HWF_…`. Shell `$HWF_<conditional>` needs the step's `when:` guards to match.
3. **Reference every declared input** — by a template, a later input's `when:`, `returns:`, `on_failure:`, or `$HWF_<name>` in shell text. Otherwise: `unused input`.
4. **`retry`** only on blocking local `run:` or `herdr:` — never on a placed, background or `on_failure` step.
5. **`target:`** requires an existing idle/done agent, and rejects `pane:`, `cwd:`, `env:` and `using:`. Warn the user if the pane that invokes the workflow may still be busy.
...
7. **`pane.size` and `pane.target` are valid only for `open: beside`/`below`**, and `pane.workspace` only for `open: tab`. A tab has no size, so "a tab at 40%" must become `open: beside, size: 40` — tell the user, do not drop the size.
...
9. **`herdr:` steps need that method's exact selector** (table in reference/herdr-api.md). The runner does not autofill from live UI focus, and a template on an unrelated param does not waive it. For an `exactly one of` method, both selectors together also fail to load.
