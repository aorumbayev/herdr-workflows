...
func ParityBaseline() []ParitySurface {
	...
	return []ParitySurface{
		...
		{Spec: presentation, Requirement: "Run detail offers retry", Scenario: "r retries all steps and f retries from the failed step", Kind: "action", GoSurface: "picker.beginRunsRetry/startLaunch", CoveringTest: "picker.TestRunsRetryKeysLaunchTheRecordedRun"},
		{Spec: presentation, Requirement: "Run detail offers retry", Scenario: "f on a succeeded run explains and does not launch", Kind: "action", GoSurface: "picker.beginRunsRetry", CoveringTest: "picker.TestRunsRetryFromFailedRefusesASucceededRun"},
		{Spec: presentation, Requirement: "Run detail offers retry", Scenario: "Retry launch shows the consent line of the current workflow", Kind: "view", GoSurface: "picker.beginRunsRetry/FormatConsentLine", CoveringTest: "picker.TestRunsRetryKeysLaunchTheRecordedRun"},
		{Spec: presentation, Requirement: "Run detail offers retry", Scenario: "Retry refuses a workflow missing from this checkout", Kind: "action", GoSurface: "picker.beginRunsRetry/entryNamed", CoveringTest: "picker.TestRunsRetryRefusesAWorkflowMissingFromThisCheckout"},
		{Spec: presentation, Requirement: "Run detail offers retry", Scenario: "Detail footer names the retry keys", Kind: "view", GoSurface: "RunDetailFooter", CoveringTest: "TestRunDetailFooterNamesRetryKeys"},
		{Spec: presentation, Requirement: "Run detail offers retry", Scenario: "Reused steps carry a reused mark", Kind: "view", GoSurface: "detailCards", CoveringTest: "TestDetailCardsMarkReusedSteps"},
