...
func ParityBaseline() []ParitySurface {
	...
	return []ParitySurface{
		...
		{Spec: presentation, Requirement: "Run detail exposes log, transcript, and yaml-at-run", Scenario: "Retry-copy copies hwf retry <id>", Kind: "action", GoSurface: "Model.handleDetailKey/FormatRetryCommand", CoveringTest: "TestModelRunDetailDebugTabsAndRetryCopy"},
		{Spec: presentation, Requirement: "Run detail exposes log, transcript, and yaml-at-run", Scenario: "Shift+Y copies hwf retry <id> --from-failed", Kind: "action", GoSurface: "Model.handleDetailKey/FormatRetryCommand", CoveringTest: "TestFormatRetryCommand"},
		{Spec: presentation, Requirement: "Run detail exposes log, transcript, and yaml-at-run", Scenario: "Detail footer names both retry-copy keys", Kind: "view", GoSurface: "detailFooter", CoveringTest: "TestModelRunDetailDebugTabsAndRetryCopy"},
		...
		{Spec: presentation, Requirement: "Diagram send-back types an annotation bundle into an agent pane", Scenario: "Chooser rows read as workspace, tab, status, agent, and pane id", Kind: "view", GoSurface: "FormatAgentPickBody/host.ParseAgentPanes", CoveringTest: "TestAgentPickBodyShowsLocationStatusAgentPaneIDAndSelfMarker"},
		{Spec: presentation, Requirement: "Diagram send-back types an annotation bundle into an agent pane", Scenario: "Chooser marks the caller's own pane", Kind: "view", GoSurface: "AgentPaneEntry.Self/FormatAgentPickBody", CoveringTest: "TestAgentPickBodyTruncatesLocationAndTitleNeverWraps"},
		{Spec: presentation, Requirement: "Diagram send-back types an annotation bundle into an agent pane", Scenario: "Chooser footer explains the status glyphs", Kind: "view", GoSurface: "tui.AgentStatusLegend/renderDiagramAgentPick", CoveringTest: "TestAgentPickFooterCarriesTheStatusLegend"},
		...
		{Spec: presentation, Requirement: "Diagram send-back types an annotation bundle into an agent pane", Scenario: "Empty composer shows a placeholder", Kind: "view", GoSurface: "composerDraft/tui.FormatField", CoveringTest: "TestComposerDraftShowsPlaceholderOnlyWhenEmpty"},
		...
		{Spec: presentation, Requirement: "Console catalog uses the overlay row language", Scenario: "Title, sensitivity marker, and repo/global/invalid location", Kind: "view", GoSurface: "Model.renderWorkflows/entryLocation/entrySensitive", CoveringTest: "TestModelConsoleCatalogChromeMatchesOverlay"},
		{Spec: presentation, Requirement: "Console catalog uses the overlay row language", Scenario: "Filter placeholder narrows the list", Kind: "view", GoSurface: "Model.visibleEntries/filterRow", CoveringTest: "TestModelConsoleFiltersWorkflows"},
		{Spec: presentation, Requirement: "Console footer names two tabs and the real doors", Scenario: "Workflows and runs footers", Kind: "view", GoSurface: "workflowsFooter/runsFooter", CoveringTest: "TestModelWorkflowsListAndTabToRuns"},
		{Spec: presentation, Requirement: "Pop-out lands on the selected workflow diagram", Scenario: "Picker carries the selection to the pane", Kind: "transition", GoSurface: "picker.consoleLandingWorkflow", CoveringTest: "picker.TestConsoleLandsOnSelectedWorkflow"},
		{Spec: presentation, Requirement: "Pop-out lands on the selected workflow diagram", Scenario: "Console opens the diagram and escape returns to the catalog", Kind: "transition", GoSurface: "Model.New/OpenDiagram", CoveringTest: "TestModelConsoleLandsOnSelectedDiagram"},
		{Spec: presentation, Requirement: "Quit restores the terminal", Scenario: "Standalone console uses alt-screen", Kind: "view", GoSurface: "Model.View AltScreen", CoveringTest: "TestModelConsoleViewUsesAltScreen"},
	}
}
