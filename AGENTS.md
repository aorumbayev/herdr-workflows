...
| `src/repo-root.ts`                           | realpath-canonical checkout root for exact Current-scope matching                                                                                                                                                                                                   |
...
| `src/workflow/`                              | types.ts, parse.ts (YAML → WorkflowStep[]), validate.ts (references, child contracts), load.ts (name → LoadedWorkflow), conditions.ts (`when:`), inputs.ts (collection order), trust.ts (authoring warnings), payload.ts (share bundle codec), export.ts, import.ts |
...
| `src/tui/`                                   | picker.ts, picker-rows.ts, picker-runs.ts, run-history-view.ts, picker-actions.ts (ctrl+k palette), run-launch.ts, theme.ts, update-indicator.ts                                                                                                                    |
| `src/web/`                                   | server.ts, endpoint.ts, route.ts, credential-store.ts (private credential dir, ACL assertions)                                                                                                                                                                      |
