...
| `src/workflow/`                              | types.ts, parse.ts (YAML → WorkflowStep[]), validate.ts (references, child contracts), load.ts (name → LoadedWorkflow), conditions.ts (`when:`), inputs.ts (collection order), trust.ts (authoring warnings), share.ts (share bundle codec), dump.ts, paths.ts, import.ts |
| `src/run/`                                   | runner.ts, context.ts (step contracts, outcomes, coordination errors), steps/ (agent, shell, primitive, pane placement, include)                                                                                                                                          |
| `src/tui/`                                   | picker.ts, picker-rows.ts, picker-actions.ts (ctrl+k palette), runs-browser.ts, run-history.ts, run-launch.ts, theme.ts, update-indicator.ts                                                                                                                              |
