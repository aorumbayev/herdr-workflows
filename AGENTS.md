...
| `src/workflow/`                              | types.ts, parse.ts (YAML → WorkflowStep[]), validate.ts (references, child contracts), load.ts (name → LoadedWorkflow), conditions.ts (`when:`), inputs.ts (collection order), trust.ts (authoring warnings), share.ts (share bundle codec), import.ts |
