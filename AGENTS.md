...
- **Loops and conditionals are in scope** (`when:`, `for:` / `as:`, `retry:`). **Parallelism is not.** Windows is supported via the `{platform}` builtin + `when:` comparisons — no other per-OS syntax. `retry:` on pane-creating steps (`agent:` or `run:` with `in:` other than `here`) requires author `reset:` — herdr has no create-or-return-by-key API.
...
- **Schema change:** edit Zod in `src/workflow/parse.ts` (and refine rules), then `bun run schema`. Method validators: update `schemas/herdr-api.schema.json` and `bun run schema:herdr` (release-time; plugin build must not invoke `herdr api schema`). Cross-field rules live in the loader, not the JSON schema.
