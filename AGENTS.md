...
| `src/console.ts`                             | CLI/TUI terminal I/O (`die`, `readLine`, `openInBrowser`, closed-stdio tolerance)                                                                                                                                                                                                                        |
| `src/herdr.ts`                               | socket RPC, CLI wrappers, protocol check, pane placement                                                                                                                                                                                                                                                 |
| `src/herdr-methods.ts`                       | method param validation and explicit-target focus policy                                                                                                                                                                                                                                                 |
| `src/herdr-methods.generated.ts`             | generated — do not hand-edit                                                                                                                                                                                                                                                                             |
| `src/config.ts`                              | profile/transcript config layers, repo root, invocation context, `loadContext`, display sanitization                                                                                                                                                                                                     |
| `src/limits.ts`                              | shared capture and environment byte caps                                                                                                                                                                                                                                                                 |
| `src/session.ts`                             | transcript extraction (config extractors + built-in)                                                                                                                                                                                                                                                     |
| `src/history/`                               | store.ts (atomic snapshots, list/detail), project.ts, recorder.ts, ack.ts (history ack codec), types.ts, retention                                                                                                                                                                                       |
| `src/init.ts`                                | `hwf init` — profile probe, config write, `.hwf/` scaffold                                                                                                                                                                                                                                               |
| `src/update.ts`                              | `hwf update` via herdr plugin install; GitHub latest-release fetch + semver compare                                                                                                                                                                                                                      |
| `src/setup.ts`                               | host setup: PATH CLI install + picker keybindings                                                                                                                                                                                                                                                        |
| `src/version.ts`                             | embedded plugin version and pinned workflow schema URL                                                                                                                                                                                                                                                   |
| `src/fs-private.ts`                          | private credential dir/file mode and ACL assertions                                                                                                                                                                                                                                                      |
| `src/latest.ts`                              | monotonic latest-wins generation token                                                                                                                                                                                                                                                                   |
| `src/workflow/`                              | types.ts, template.ts (template engine), parse.ts (YAML → WorkflowStep[]), validate.ts (references, child contracts), load.ts (name → LoadedWorkflow), conditions.ts (`when:`), inputs.ts (collection order), trust.ts (authoring warnings), share.ts (share bundle codec), dump.ts, paths.ts, import.ts |
| `src/run/`                                   | runner.ts, launch.ts (identity, payload, retire-on-code-change), context.ts (step contracts, outcomes, coordination errors, herdr step), steps/ (agent, shell, pane placement, include)                                                                                                                  |
| `src/tui/`                                   | picker.ts, picker-rows.ts, picker-actions.ts (ctrl+k palette), picker-chrome.ts (sole `@opentui/core` importer), runs-browser.ts, run-history.ts, update-indicator.ts                                                                                                                                    |
| `src/web/`                                   | server.ts, endpoint.ts (workbench adopt/lock), field-model.ts (canvas field metadata)                                                                                                                                                                                                                    |
| `src/web/page.html`                          | workbench UI served by the web server                                                                                                                                                                                                                                                                    |
| `src/svg-text.d.ts`                          | ambient `*.svg` text import used by `src/web/server.ts`                                                                                                                                                                                                                                                  |
...
- **Module layers.** Surfaces (`cli`, `tui`, `web`) → domain (`workflow`, `run`, `history`) → platform (`herdr`, `config`, `limits`, `session`, and peers). Imports only point down. The only sanctioned sideways/up edge is `run` (engine) → `history`. Cross-module imports go through each module's entry files; `verify:layers` enforces this.
...
- **Caps live in `src/limits.ts`:** 24 KiB generated HWF environment (entry and child, before step 1), 8 MiB per captured command result / managed response / transcript / dynamic-choice output, 16 KiB agent prompt before spill-to-file. Crossing a cap fails naming source and limit — never truncate.
...
- **Splitting:** `verify:complexity --threshold 12` scores `171 − 5.2·ln(HalsteadVolume) − 0.23·CyclomaticComplexity − 16.2·ln(SLOC)` per function, but with the **whole file's** SLOC — so a file's score (its lowest function) drops both as the file grows and as its worst function gets gnarlier. Either shrinks it back. `src/workflow/parse.ts` sits at 13.0, one point above the gate: any file that grows or gains a gnarlier function fails. Nesting and callback depth are capped per function by oxlint (`max-depth` 4, `max-nested-callbacks` 3) — those are the readability gates. Function length is deliberately ungated. Never split to satisfy a line budget.
