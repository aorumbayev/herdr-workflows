...
Cobra MUST validate argv before command handlers run. After successful parsing, `run`, `picker`, `console`, and `launch` MUST call Herdr protocol and minimum-version preflight before reading a launch payload, resolving workflow inputs, executing a workflow, or loading picker or console UI. A protocol mismatch or too-old version MUST fail naming installed and required values before any workflow missing-input or execution error.
